...
rIUIG<-function (n, mu = 0.7, sigma = 2.1, nu = 0.1){
  ...
  r <- rep(NA, n)
  for (i in 1:n) {
    r[i] <- qIUIG(p[i], mu[i], sigma[i], nu[i])
  }
  r
...
# O problema é quando o meu p gerado é menor que meu nu, sendo assim da um valor 
#negativo para u, gerando problema na quantílica
